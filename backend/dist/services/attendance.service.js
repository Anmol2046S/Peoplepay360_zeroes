...
class AttendanceService {
    ...
    static async getActiveSession(employeeId) {
        const active = await database_1.prisma.attendance.findFirst({
            where: {
                employeeId,
                checkOut: null,
            },
    ...
    static async checkIn(employeeId, timestamp) {
        ...
        if (activeSession) {
            throw new apiResponse_1.AppError('You are already checked in. Please check out first.', 400, 'ALREADY_CHECKED_IN');
        }
        ...
        if (existing && existing.checkOut !== null) {
            throw new apiResponse_1.AppError('Attendance record already exists for today.', 400, 'ATTENDANCE_EXISTS');
        }
        const record = await database_1.prisma.attendance.create({
            data: {
                employeeId,
                date: dateOnly,
                checkIn: checkInDate,
    ...
    static async checkOut(employeeId, timestamp) {
        const activeSession = await this.getActiveSession(employeeId);
        if (!activeSession) {
            throw new apiResponse_1.AppError('No active check-in session found to check out.', 400, 'NO_ACTIVE_SESSION');
        }
        ...
        if (workedHours > expectedHours) {
            ...
            status = client_1.AttendanceStatus.OVERTIME;
        }
    ...
    static async updateAttendanceRecord(id, data) {
        const record = await database_1.prisma.attendance.findUnique({ where: { id } });
        if (!record) {
            throw new apiResponse_1.AppError('Attendance record not found.', 404, 'RECORD_NOT_FOUND');
        }
