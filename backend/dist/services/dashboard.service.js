...
class DashboardService {
    static async getMetrics(query) {
        ...
        const timeOffReqs = await database_1.prisma.timeOffRequest.findMany({
            where: { status: 'APPROVED' },
            select: { durationDays: true },
        });
        const approvedTimeOffDays = timeOffReqs.reduce((acc, r) => acc + r.durationDays, 0);
        // 3. Attendance Health
        const attendances = await database_1.prisma.attendance.findMany({
            select: { status: true },
        });
    ...
    static async getOperationalAlerts() {
        // Missing bank info
        const missingBankCount = await database_1.prisma.employee.count({
            where: {
                OR: [
        ...
        const draftPayrunCount = await database_1.prisma.payrun.count({
            where: { status: 'DRAFT' },
        });
        // Contracts expiring soon
        const now = new Date();
        const thirtyDaysLater = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
        const expiringContractsCount = await database_1.prisma.contract.count({
            where: {
                status: 'RUNNING',
                endDate: {
                    gte: now,
