...
class DashboardController {
    async getMetrics(request, reply) {
        ...
        try {
            ...
            const pendingApprovals = await db_1.prisma.timeOffRequest.count({
                where: {
                    employee: { orgId },
                    status: 'PENDING'
                }
            ...
            const latestPayrun = await db_1.prisma.payrun.findFirst({
                where: { orgId },
            ...
            const onLeaveToday = await db_1.prisma.timeOffRequest.count({
                where: {
                    employee: { orgId },
                    status: 'APPROVED',
                    startDate: { lte: today },
            ...
            for (let i = 4; i >= 0; i--) {
                ...
                const present = await db_1.prisma.attendance.count({
                    where: {
                        employee: { orgId },
                ...
                const absent = await db_1.prisma.attendance.count({
                    where: {
                        employee: { orgId },
                        date: { gte: d, lt: nextDay },
                        status: 'ABSENT'
